...
# ------------------------------------------------------
# ROUTE 2: OTP Verification Page (GET)
# ------------------------------------------------------
...
# ------------------------------------------------------
# ROUTE 3: Verify OTP & Insert Admin into DB
# ------------------------------------------------------
...
# =================================================================
# ROUTE 27: CREATE RAZORPAY ORDER
# =================================================================
@app.route('/user/pay')
def user_pay():
    ...
    cursor.execute("SELECT * FROM user_address WHERE user_id=%s", (session['user_id'],))
    address = cursor.fetchone()

    # No address found → ask user to add address first
    if not address:
        flash("Please add your delivery address first!", "warning")
        return redirect('/user/add-address')
    
    # -------------------------------
    # ADDRESS EXISTS — CONTINUE PAYMENT
    # -------------------------------
    ...
    return render_template(
        ...
        address=address  # OPTIONAL: If you want to show address on payment page
    )
...
# =================================================================
#  route: 28   TEMP SUCCESS PAGE (Verification in Day 13)
# =================================================================
...
def verify_payment():
    ...
    # -----------------------------------
    # 2. Read payment details from frontend
    # -----------------------------------
    ...
    # Build verification payload required by Razorpay
    payload = {
    ...
    # -----------------------------------
    # 5. Fetch user address (required for order)
    # -----------------------------------
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    cursor2.execute("SELECT * FROM user_address WHERE user_id=%s", (user_id,))
    addr = cursor2.fetchone()
    cursor2.close()

    # Build readable address string
    full_address = f"{addr['full_name']}, {addr['phone']}, {addr['house']}, {addr['street']}, {addr['city']}, {addr['state']} - {addr['pincode']}"
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO orders 
            (user_id, razorpay_order_id, razorpay_payment_id, amount, payment_status, order_status, address)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """,
        (user_id, razorpay_order_id, razorpay_payment_id, total_amount, 'paid', 'Pending', full_address))

        # Get the unique order_id
        order_db_id = cursor.lastrowid

        # -----------------------------------
        # 7. Insert each item from the cart
        # -----------------------------------
        ...
        # -----------------------------------
        # 9. Clear session data after order is completed
        # -----------------------------------
...
#Route-32:  My Orders Page (List user's orders)
...
#Route:33
#🟨 4. Add Route to Generate & Download PDF
...
def add_address():
    ...
    if request.method == 'POST':
        ...
        cursor.execute("""
            INSERT INTO user_address (user_id, full_name, phone, house, street, city, state, pincode)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
        """, (session['user_id'], full_name, phone, house, street, city, state, pincode))
